...
static void set_rtk_vid(int vid, int add, char *flag)
{
	FILE *f = fopen("/proc/rtk_vlan_vconfig", "w");
	if(f) {
		fprintf(f, "%s %d %s", add ? "add" : "del", vid, add ? (flag ? flag : "") : "");
	fclose(f);
	}
}
	...
	if (ifr.cmd == SET_VLAN_NAME_TYPE_CMD) {
		...
		if (ifr.cmd == SET_VLAN_FLAG_CMD) {
			...
			if(strchr(p, '.')) {
				...
				if(f)
					flag = strdup(f);
				ifr.u.VID = xatou_range(tmp, 0, VLAN_GROUP_ARRAY_LEN - 1);
				free(tmp);
			} else {
				ifr.u.VID = xatou_range(p, 0, VLAN_GROUP_ARRAY_LEN - 1);
			}
			set_rtk_vid(ifr.u.VID, 1, flag);
			if (flag) free(flag);
		...
		if (ifr.cmd == DEL_VLAN_CMD) {
			char *tmp = strdup(ifr.device1);
			char *x = strchr(tmp, '.');
			int vid = -1;
			int l = strlen(tmp);
			if (x) {
