...
static int64_t udhcpd_current_time;
static int64_t udhcpd_prev_time;
static int udhcpd_get_time_first = 1;
static int64_t udhcpd_timeshift;

void FAST_FUNC update_leases_times(int64_t timeshift, int64_t prev_time, int64_t new_time);
void FAST_FUNC write_leases_human_readable(int64_t force_time);

static void check_timeshift(void)
{
	udhcpd_current_time = time(NULL);
	if (udhcpd_get_time_first) {
		udhcpd_get_time_first = 0;
		udhcpd_prev_time = udhcpd_current_time;
		return;
	}
	if (udhcpd_current_time > udhcpd_prev_time &&
	   (udhcpd_current_time - udhcpd_prev_time) >= 100000)
	{
		udhcpd_timeshift = udhcpd_current_time - udhcpd_prev_time;
		bb_error_msg("Timeshift detected, value is %ld\n", udhcpd_timeshift);
		update_leases_times(udhcpd_timeshift, udhcpd_prev_time, udhcpd_current_time);
		write_leases_human_readable(udhcpd_current_time);
	}
	udhcpd_prev_time = udhcpd_current_time;
}
...
static const struct config_keyword keywords[] = {
	...
	{"auto_time"    , read_u32        , OFS(auto_time         ), "10"},
	...
	{"router"       , udhcp_str2nip   , OFS(router            ), "192.168.0.1"},
	{"dns"          , udhcp_str2nip   , OFS(dns               ), "192.168.0.1"},
	{"netmask"      , udhcp_str2nip   , OFS(netmask           ), "255.255.255.0"},
	{"lease_time"   , read_u32        , OFS(default_lease_time), "720"},
};
...
void FAST_FUNC update_leases_times(int64_t timeshift UNUSED_PARAM, int64_t prev_time, int64_t new_time)
{
	unsigned i;
	for (i = 0; i < server_config.max_leases; i++) {
		int64_t r;
		if (g_leases[i].lease_nip == 0)
			continue;
		r = g_leases[i].expires - prev_time;
		g_leases[i].expires = r + new_time;
	}
}

/* Refine write_leases() function for human-readable format. */
#if 0
...
{
	unsigned i;
	FILE *f = fopen(server_config.lease_file, "w+");
	int64_t current_time = force_time ? force_time : time(NULL);
	if (f) {
		for (i = 0; i < server_config.max_leases; i++) {
			int64_t lease_time;
			unsigned char *x;
			if (g_leases[i].lease_nip == 0)
				continue;
			lease_time = g_leases[i].expires;
			if (lease_time < current_time)
				continue;
			fprintf(f, "%u ", g_leases[i].expires);
			fprintf(f, "%02X:%02X:%02X:%02X:%02X:%02X ", g_leases[i].lease_mac[0],
				g_leases[i].lease_mac[1], g_leases[i].lease_mac[2], g_leases[i].lease_mac[3],
				g_leases[i].lease_mac[4], g_leases[i].lease_mac[5]);
			x = (unsigned char *)&g_leases[i].lease_nip;
			fprintf(f, "%d.%d.%d.%d ", x[0], x[1], x[2], x[3]);
			fprintf(f, "%s ", (g_leases[i].hostname && strlen(g_leases[i].hostname)) ? g_leases[i].hostname : "unknown");
			fprintf(f, "\n");
		}
		fclose(f);
	}
...
static void FAST_FUNC write_leases(void)
{
	write_leases_human_readable(0);
}
#endif

/*
	Preblagin: we refined write_leases() function for human-readable format.
	Do the same thing for read_leases().
*/
#if 0
...
#else
static int FAST_FUNC read_single_lease(FILE *f, struct dyn_lease *lease)
{
	char buf[1024] = {0};
	memset(lease, 0, sizeof(struct dyn_lease));
	if (fgets(buf, sizeof(buf), f) != NULL && ferror(f) == 0) {
		/* BIG ENDIAN :( */
		unsigned int nip[4] = {0};
		unsigned int mac[6] = {0};

		/* fmt: expired MAC IP hostname */
		int n = sscanf(buf, "%u %x:%x:%x:%x:%x:%x %d.%d.%d.%d %s", &lease->expires,
			&mac[0], &mac[1], &mac[2], &mac[3], &mac[4], &mac[5],
			&nip[0], &nip[1], &nip[2], &nip[3],
			lease->hostname);

		if (n == 12) {
			unsigned char *tmp = (unsigned char *)&(lease->lease_nip);
			for (n = 0; n < 4; n++)
				tmp[n] = nip[n];
			for (n = 0; n < 6; n++)
				lease->lease_mac[n] = mac[n];
			return 0;
		}
	}
	return -1;
...
static void FAST_FUNC read_leases(const char *file)
{
	struct dyn_lease lease;
	FILE *lease_file = NULL;
	lease_file = fopen(file, "r");
	if (lease_file == NULL)
		return;
	while (read_single_lease(lease_file, &lease) == 0) {
		leasetime_t cur_time = time(NULL);
		uint32_t y = ntohl(lease.lease_nip);

		if (cur_time >= lease.expires || y < server_config.start_ip || y > server_config.end_ip) {
			continue;
		}

		if (add_lease(lease.lease_mac, lease.lease_nip, lease.expires - cur_time,
		...
		unsigned arpping_ms)
{
	...
	udhcp_add_simple_option(&packet, DHCP_ROUTER, server_config.router);
	udhcp_add_simple_option(&packet, DHCP_DNS_SERVER, server_config.dns);
	udhcp_add_simple_option(&packet, DHCP_SUBNET, server_config.netmask);
...
int udhcpd_main(int argc UNUSED_PARAM, char **argv)
{
	...
	server_config.max_lease_sec = server_config.default_lease_time;
	if (option) {
...
*/
			...
			if (server_id_opt    /* client is in SELECTING state */
			 ...
			 || ((packet.flags & htons(BROADCAST_FLAG)) == 0) /* Preblagin: unicast pkt for us, #50865 */
			) {
