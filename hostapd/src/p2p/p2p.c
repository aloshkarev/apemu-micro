...
void p2p_expire_peers(struct p2p_data *p2p) {
    ...
    os_get_reltime(&now);
    dl_list_for_each_safe(dev, n, &p2p->devices,
        ...
        p2p_dbg(p2p, "Expiring old peer entry "
                     MACSTR,
...
int p2p_add_device(struct p2p_data *p2p, const u8 *addr, int freq,
    ...
    if (!is_zero_ether_addr(p2p->peer_filter) &&
        ...
        p2p_dbg(p2p, "Do not add peer filter for "
                     MACSTR
    ...
    if (dev->info.config_methods == 0 &&
		 ...
		 * information.
		 */
        p2p_dbg(p2p, "Do not report peer "
                     MACSTR
...
static int p2p_run_after_scan(struct p2p_data *p2p) {
    ...
    switch (op) {
        ...
        case P2P_AFTER_SCAN_CONNECT:
            p2p_dbg(p2p, "Start previously requested connect with "
                         MACSTR,
...
int p2p_find(struct p2p_data *p2p, unsigned int timeout,
    ...
    if (seek_count == 0 || !seek) {
        ...
        for (i = 0; i < seek_count; i++) {
            ...
            p2p_dbg(p2p, "Seek service %s hash "
                         MACSTR,
                    seek[i], MAC2STR(buf));
...
int p2p_connect(struct p2p_data *p2p, const u8 *peer_addr,
    ...
    p2p_dbg(p2p, "Request to start group negotiation - peer="
                 MACSTR
                 "  GO Intent=%d  Intended Interface Address="
                 MACSTR
    ...
    if (dev == NULL || (dev->flags & P2P_DEV_PROBE_REQ_ONLY)) {
        p2p_dbg(p2p, "Cannot connect to unknown P2P Device "
                     MACSTR,
                MAC2STR(peer_addr));
        return -1;
    }
    ...
    if (dev->flags & P2P_DEV_GROUP_CLIENT_ONLY) {
        if (!(dev->info.dev_capab &
              P2P_DEV_CAPAB_CLIENT_DISCOVERABILITY)) {
            p2p_dbg(p2p, "Cannot connect to P2P Device "
                         MACSTR
...
int p2p_authorize(struct p2p_data *p2p, const u8 *peer_addr,
    ...
    p2p_dbg(p2p, "Request to authorize group negotiation - peer="
                 MACSTR
                 "  GO Intent=%d  Intended Interface Address="
                 MACSTR
    ...
    dev = p2p_get_device(p2p, peer_addr);
    if (dev == NULL) {
        p2p_dbg(p2p, "Cannot authorize unknown P2P Device "
                     MACSTR,
                MAC2STR(peer_addr));
        return -1;
    }
...
void p2p_go_complete(struct p2p_data *p2p, struct p2p_device *peer) {
    ...
    p2p_dbg(p2p, "GO Negotiation with "
                 MACSTR
...
static void p2p_rx_p2p_action(struct p2p_data *p2p, const u8 *sa,
    ...
    p2p_dbg(p2p, "RX P2P Public Action from "
                 MACSTR, MAC2STR(sa));
...
void p2p_rx_action(struct p2p_data *p2p, const u8 *da, const u8 *sa,
    ...
    /* P2P action frame */
    p2p_dbg(p2p, "RX P2P Action from "
                 MACSTR, MAC2STR(sa));
...
static void p2p_add_dev_from_probe_req(struct p2p_data *p2p, const u8 *addr,
    ...
    p2p_dbg(p2p, "Created device entry based on Probe Req: "
                 MACSTR
...
p2p_reply_probe(struct p2p_data *p2p, const u8 *addr, const u8 *dst,
    ...
    if (msg.device_id &&
        ...
        p2p_dbg(p2p, "Probe Req requested Device ID "
                     MACSTR
...
void p2p_wps_success_cb(struct p2p_data *p2p, const u8 *mac_addr) {
    ...
    p2p_dbg(p2p, "Group Formation completed successfully with "
                 MACSTR,
            MAC2STR(mac_addr));
...
void p2p_group_formation_failed(struct p2p_data *p2p) {
    ...
    p2p_dbg(p2p, "Group Formation failed with "
                 MACSTR,
            MAC2STR(p2p->go_neg_peer->intended_addr));
...
void p2p_flush(struct p2p_data *p2p) {
    ...
    p2p_stop_find(p2p);
    dl_list_for_each_safe(dev, prev, &p2p->devices,
                          struct p2p_device,
                          list) {
        dl_list_del(&dev->list);
        p2p_device_free(p2p, dev);
...
int p2p_scan_res_handler(struct p2p_data *p2p, const u8 *bssid, int freq,
    ...
    if (os_reltime_before(rx_time, &p2p->find_start)) {
		 ...
		 */
        p2p_dbg(p2p, "Ignore old scan result for "
                     MACSTR
...
void p2p_send_action_cb(struct p2p_data *p2p, unsigned int freq, const u8 *dst,
    ...
    p2p_dbg(p2p, "Action frame TX callback (state=%d freq=%u dst="
                 MACSTR
                 " src="
                 MACSTR
                 " bssid="
...
int p2p_presence_req(struct p2p_data *p2p, const u8 *go_interface_addr,
    ...
    p2p_dbg(p2p, "Send Presence Request to GO "
                 MACSTR
                 " (own interface "
                 MACSTR
...
void p2p_deauth_notif(struct p2p_data *p2p, const u8 *bssid, u16 reason_code,
    ...
    p2p_dbg(p2p, "Deauthentication notification BSSID "
                 MACSTR
...
void p2p_disassoc_notif(struct p2p_data *p2p, const u8 *bssid, u16 reason_code,
    ...
    p2p_dbg(p2p, "Disassociation notification BSSID "
                 MACSTR
                 " reason_code=%u minor_reason_code=%u",
            MAC2STR(bssid), reason_code, *msg.minor_reason_code);
...
void p2p_set_peer_filter(struct p2p_data *p2p, const u8 *addr) {
    ...
    if (is_zero_ether_addr(p2p->peer_filter))
        ...
        p2p_dbg(p2p, "Enable peer filter for "
                     MACSTR,
