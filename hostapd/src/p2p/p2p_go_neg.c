...
int p2p_connect_send(struct p2p_data *p2p, struct p2p_device *dev) {
    ...
    if (dev->flags & P2P_DEV_PD_BEFORE_GO_NEG) {
        u16 config_method;
        p2p_dbg(p2p, "Use PD-before-GO-Neg workaround for "
                     MACSTR,
...
int p2p_go_select_channel(struct p2p_data *p2p, struct p2p_device *dev,
    ...
    if (!p2p_channels_includes(&intersection, p2p->op_reg_class,
        ...
        p2p_dbg(p2p,
                "Try to optimize channel selection with peer information received; previously selected op_class %u channel %u",
                p2p->op_reg_class, p2p->op_channel);
        p2p_reselect_channel(p2p, &intersection);
    }
...
void p2p_process_go_neg_req(struct p2p_data *p2p, const u8 *sa,
    ...
    p2p_dbg(p2p, "Received GO Negotiation Request from "
                 MACSTR
                 "(freq=%d)",
            MAC2STR(sa), rx_freq);
    ...
    if (os_memcmp(msg.p2p_device_addr, sa, ETH_ALEN) != 0) {
        p2p_dbg(p2p, "Unexpected GO Negotiation Request SA="
                     MACSTR
                     " != dev_addr="
                     MACSTR,
        ...
        status = P2P_SC_FAIL_REJECTED_BY_USER;
               ...
               (dev->wps_method == WPS_NOT_READY &&
                (p2p->authorized_oob_dev_pw_id == 0 ||
                 ...
                 msg.dev_password_id))) {
        p2p_dbg(p2p, "Not ready for GO negotiation with "
                     MACSTR,
                MAC2STR(sa));
...
void p2p_process_go_neg_resp(struct p2p_data *p2p, const u8 *sa,
    ...
    p2p_dbg(p2p, "Received GO Negotiation Response from "
                 MACSTR
...
void p2p_process_go_neg_conf(struct p2p_data *p2p, const u8 *sa,
    ...
    p2p_dbg(p2p, "Received GO Negotiation Confirm from "
                 MACSTR,
            MAC2STR(sa));
    dev = p2p_get_device(p2p, sa);
    if (dev == NULL || dev->wps_method == WPS_NOT_READY ||
