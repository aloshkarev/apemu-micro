...
struct p2p_sd_query *p2p_pending_sd_req(struct p2p_data *p2p,
    ...
    found:
    if (dev->sd_reqs > 100) {
        p2p_dbg(p2p, "Too many SD request attempts to "
                     MACSTR
...
int p2p_start_sd(struct p2p_data *p2p, struct p2p_device *dev) {
    ...
    if (p2p->state == P2P_SEARCH &&
        os_memcmp(p2p->sd_query_no_ack, dev->info.p2p_device_addr,
                  ETH_ALEN) == 0) {
        p2p_dbg(p2p, "Do not start Service Discovery with "
                     MACSTR
    ...
    p2p_dbg(p2p, "Start Service Discovery with "
                 MACSTR,
            MAC2STR(dev->info.p2p_device_addr));
...
void p2p_rx_gas_initial_req(struct p2p_data *p2p, const u8 *sa,
    ...
    dialog_token = *pos++;
    p2p_dbg(p2p, "GAS Initial Request from "
                 MACSTR
...
void p2p_rx_gas_initial_resp(struct p2p_data *p2p, const u8 *sa,
    ...
    p2p_dbg(p2p, "Received GAS Initial Response from "
                 MACSTR
                 " (len=%d)",
            MAC2STR(sa), (int) len);
...
void p2p_rx_gas_comeback_req(struct p2p_data *p2p, const u8 *sa,
    ...
    if (os_memcmp(sa, p2p->sd_resp_addr, ETH_ALEN) != 0) {
        p2p_dbg(p2p, "No pending SD response fragment for "
                     MACSTR,
                MAC2STR(sa));
        return;
    }
...
void p2p_rx_gas_comeback_resp(struct p2p_data *p2p, const u8 *sa,
    ...
    p2p_dbg(p2p, "Received GAS Comeback Response from "
                 MACSTR
                 " (len=%d)",
            MAC2STR(sa), (int) len);
