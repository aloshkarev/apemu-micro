...
void p2p_process_prov_disc_req(struct p2p_data *p2p, const u8 *sa,
    ...
    p2p_dbg(p2p, "Received Provision Discovery Request from "
                 MACSTR
...
void p2p_process_prov_disc_resp(struct p2p_data *p2p, const u8 *sa,
    ...
    p2p_dbg(p2p, "Received Provision Discovery Response from "
                 MACSTR
    ...
    if (dev == NULL || !dev->req_config_methods) {
        p2p_dbg(p2p, "Ignore Provision Discovery Response from "
                     MACSTR
...
int p2p_send_prov_disc_req(struct p2p_data *p2p, struct p2p_device *dev,
    ...
    if (dev->flags & P2P_DEV_GROUP_CLIENT_ONLY) {
        if (!(dev->info.dev_capab &
              P2P_DEV_CAPAB_CLIENT_DISCOVERABILITY)) {
            p2p_dbg(p2p, "Cannot use PD with P2P Device "
                         MACSTR
...
int p2p_prov_disc_req(struct p2p_data *p2p, const u8 *peer_addr,
    ...
    if (dev == NULL || (dev->flags & P2P_DEV_PROBE_REQ_ONLY)) {
        p2p_dbg(p2p, "Provision Discovery Request destination "
                     MACSTR
    ...
    p2p_dbg(p2p, "Provision Discovery Request with "
                 MACSTR
                 " (config methods 0x%x)",
