...
void p2p_process_invitation_req(struct p2p_data *p2p, const u8 *sa,
    ...
    p2p_dbg(p2p, "Received Invitation Request from "
                 MACSTR
                 " (freq=%d)",
            MAC2STR(sa), rx_freq);
    ...
    if (dev == NULL || (dev->flags & P2P_DEV_PROBE_REQ_ONLY)) {
        p2p_dbg(p2p, "Invitation Request from unknown peer "
                     MACSTR,
                MAC2STR(sa));
    ...
    if (op_freq) {
        ...
        if (go &&
            !p2p_channels_includes(&intersection, p2p->op_reg_class,
                                   p2p->op_channel)) {
            p2p_dbg(p2p,
                    "Initially selected channel (op_class %d channel %d) not in channel intersection - try to reselect",
                    p2p->op_reg_class, p2p->op_channel);
            ...
            p2p_dbg(p2p,
                    "Try to reselect channel selection with peer information received; previously selected op_class %u channel %u",
                    p2p->op_reg_class, p2p->op_channel);
            p2p_reselect_channel(p2p, &intersection);
        }
...
void p2p_process_invitation_resp(struct p2p_data *p2p, const u8 *sa,
    ...
    p2p_dbg(p2p, "Received Invitation Response from "
                 MACSTR,
            MAC2STR(sa));
...
int p2p_invite(struct p2p_data *p2p, const u8 *peer, enum p2p_invite_role role,
    ...
    p2p_dbg(p2p, "Request to invite peer "
                 MACSTR
                 " role=%d persistent=%d "
            ...
            p2p->allow_6ghz);
    if (bssid)
        p2p_dbg(p2p, "Invitation for BSSID "
                     MACSTR, MAC2STR(bssid));
    if (go_dev_addr) {
        p2p_dbg(p2p, "Invitation for GO Device Address "
                     MACSTR,
                MAC2STR(go_dev_addr));
    ...
    if (dev == NULL || (dev->listen_freq <= 0 && dev->oper_freq <= 0 &&
        ...
        p2p_dbg(p2p, "Cannot invite unknown P2P Device "
                     MACSTR,
                MAC2STR(peer));
        return -1;
    }
    ...
    if (dev->flags & P2P_DEV_GROUP_CLIENT_ONLY) {
        if (!(dev->info.dev_capab &
            ...
            p2p_dbg(p2p, "Cannot invite a P2P Device "
                         MACSTR
