...
#else /* UNICODE */
wpa_printf(MSG_DEBUG,
"SCARD: Selected reader='%s'", &readers[pos]);
#endif /* UNICODE */
...
if (
_scard_select_file(scard,
SCARD_FILE_MF, buf, &blen,
SCARD_USIM, NULL, 0)) {
wpa_printf(MSG_DEBUG,
...
wpa_printf(MSG_DEBUG,
"SCARD: fetching PIN retry counter");
...
if (len != 2) {
wpa_printf(MSG_WARNING,
"SCARD: failed to fetch PIN retry "
"counter");
return -1;
}
...
if (
scard_select_file(scard,
SCARD_FILE_GSM_EF_IMSI, buf, &blen))
return -1;
if (blen < 4) {
wpa_printf(MSG_WARNING,
...
int file_size;
if (
scard_parse_fsp_templ(buf, blen, NULL, &file_size))
return -3;
blen = file_size;
}
if (blen < 2 || blen > sizeof(buf)) {
wpa_printf(MSG_DEBUG,
"SCARD: invalid IMSI file length=%ld",
(long) blen);
return -3;
}
...
imsilen = (blen - 2) * 2 + 1;
wpa_printf(MSG_DEBUG,
"SCARD: IMSI file length=%ld imsilen=%ld",
...
if (
scard_read_file(scard, buf, blen
))
return -5;
...
if (file_size == 3) {
wpa_printf(MSG_DEBUG,
"SCARD: MNC length not available");
return -7;
}
if (

file_size<4 || file_size>(int)

sizeof(buf)) {
wpa_printf(MSG_DEBUG,
"SCARD: invalid file length=%ld",
(long) file_size);
return -4;
}

if (
scard_read_file(scard, buf, file_size
))
return -5;
buf[3] = buf[3] & 0x0f; /* upper nibble reserved for future use  */
if (buf[3] < 2 || buf[3] > 3) {
wpa_printf(MSG_DEBUG,
"SCARD: invalid MNC length=%ld",
(long) buf[3]);
return -6;
}
wpa_printf(MSG_DEBUG,
"SCARD: MNC length=%ld", (long) buf[3]);
return buf[3];
}
...
wpa_hexdump(MSG_DEBUG,
"SCARD: GSM auth - SRES", sres, 4);
wpa_hexdump(MSG_DEBUG,
"SCARD: GSM auth - Kc", kc, 8);
...
wpa_hexdump(MSG_DEBUG,
"SCARD: UMTS auth - RAND", _rand, AKA_RAND_LEN);
wpa_hexdump(MSG_DEBUG,
"SCARD: UMTS auth - AUTN", autn, AKA_AUTN_LEN);
cmd[5] = AKA_RAND_LEN;
...
if (len <= sizeof(resp))
wpa_hexdump(MSG_DEBUG,
"SCARD: UMTS alg response", resp, len);
...
wpa_hexdump(MSG_DEBUG,
"SCARD: UMTS get response result", buf, len);
