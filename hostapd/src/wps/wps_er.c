...
static struct wps_er_sta *wps_er_sta_get(struct wps_er_ap *ap, const u8 *addr,
    ...
    dl_list_for_each(sta, &ap->sta,
                     struct wps_er_sta, list) {
        if ((addr == NULL ||
...
static void wps_er_sta_remove_all(struct wps_er_ap *ap) {
    ...
    dl_list_for_each_safe(sta, prev, &ap->sta,
                          struct wps_er_sta, list)wps_er_sta_free(sta);
}
...
static struct wps_er_ap_settings *wps_er_ap_get_settings(struct wps_er *er,
    ...
    dl_list_for_each(s, &er->ap_settings,
                     struct wps_er_ap_settings, list)if (os_memcmp(uuid, s->uuid, WPS_UUID_LEN) == 0)
            return s;
    return NULL;
}
...
static void wps_er_ap_remove_all(struct wps_er *er) {
    ...
    dl_list_for_each_safe(ap, prev, &er->ap,
                          struct wps_er_ap, list)wps_er_ap_remove_entry(er, ap);
    dl_list_for_each_safe(s, prev_s, &er->ap_settings,
                          struct wps_er_ap_settings, list)os_free(s);
}
...
static void wps_er_sta_send_msg(struct wps_er_sta *sta, struct wpabuf *msg) {
    ...
    wpabuf_printf(buf, "<NewWLANEventMAC>"
                       MACSTR
                       "</NewWLANEventMAC>\n",
                  MAC2STR(sta->addr));
...
static void wps_er_process_wlanevent_eap(struct wps_er_ap *ap, const u8 *addr,
    ...
    wpa_printf(MSG_DEBUG, "WPS ER: WLANEvent - EAP - from "
                          MACSTR,
               MAC2STR(addr));
    wpa_hexdump_buf(MSG_MSGDUMP, "WPS ER: WLANEvent - Enrollee's message "
...
void wps_er_refresh(struct wps_er *er) {
    ...
    dl_list_for_each(ap, &er->ap,
        ...
        dl_list_for_each(sta, &ap->sta,
                         struct wps_er_sta, list)wps_er_sta_event(er->wps, sta, WPS_EV_ER_ENROLLEE_ADD);
    }
...
static void wps_er_deinit_finish(void *eloop_data, void *user_ctx) {
    ...
    dl_list_for_each_safe(ap, tmp, &er->ap_unsubscribing,
                          struct wps_er_ap,
                          list) {
