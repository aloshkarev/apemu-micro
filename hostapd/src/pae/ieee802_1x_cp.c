...
enum connect_type {
    PENDING, UNAUTHENTICATED, AUTHENTICATED, SECURE
};
...
SM_STATE(CP, INIT
    ...
    sm->
            controlled_port_enabled = false;
    secy_cp_control_enable_port(sm
                                        ->kay, sm->controlled_port_enabled);
 ...
 */
    ...
    sm->
            oan = 0;
    ieee802_1x_kay_set_old_sa_attr(sm
                                           ->kay, sm->oki, sm->oan,
                                   sm->otx, sm->orx);
    os_free(sm
                    ->lki);
    ...
    sm->
            lan = 0;
    ieee802_1x_kay_set_latest_sa_attr(sm
                                              ->kay, sm->lki, sm->lan,
                                      sm->ltx, sm->lrx);
}
...
SM_STATE(CP, ALLOWED
    ...
    sm->
            validate_frames = Checked;

    sm->
            port_valid = false;
    sm->
            controlled_port_enabled = true;

    secy_cp_control_enable_port(sm
                                        ->kay, sm->controlled_port_enabled);
    secy_cp_control_protect_frames(sm
                                           ->kay, sm->protect_frames);
    secy_cp_control_encrypt(sm
                                    ->kay, sm->kay->macsec_encrypt);
    secy_cp_control_validate_frames(sm
                                            ->kay, sm->validate_frames);
    secy_cp_control_replay(sm
                                   ->kay, sm->replay_protect, sm->replay_window);
}
...
SM_STATE(CP, SECURED
    ...
    sm->
            protect_frames = sm->kay->macsec_protect;
    sm->
            replay_protect = sm->kay->macsec_replay_protect;
    sm->
            validate_frames = sm->kay->macsec_validate;

    sm->
            current_cipher_suite = sm->cipher_suite;
    secy_cp_control_current_cipher_suite(sm
                                                 ->kay, sm->current_cipher_suite);

    sm->
            confidentiality_offset = sm->cipher_offset;

    sm->
            port_valid = true;

    secy_cp_control_confidentiality_offset(sm
                                                   ->kay,
...
SM_STATE(CP, RECEIVE
    ...
    sm->
            lki = os_malloc(sizeof(*sm->lki));
    if (!sm->lki) {
        wpa_printf(MSG_ERROR,
                   "CP-%s: Out of memory", __func__);
        return;
    }
    os_memcpy(sm
                      ->lki, &sm->distributed_ki, sizeof(*sm->lki));
    sm->
            lan = sm->distributed_an;
    sm->
            ltx = false;
    sm->
            lrx = false;
    ieee802_1x_kay_set_latest_sa_attr(sm
                                              ->kay, sm->lki, sm->lan,
                                      sm->ltx, sm->lrx);
    ieee802_1x_kay_create_sas(sm
                                      ->kay, sm->lki);
    ieee802_1x_kay_enable_rx_sas(sm
                                         ->kay, sm->lki);
    sm->
            new_sak = false;
    sm->
...
SM_STATE(CP, RECEIVING
    ...
    ieee802_1x_kay_set_latest_sa_attr(sm
                                      ...
                                      sm->ltx, sm->lrx);
    sm->
            transmit_when = sm->transmit_delay;
    eloop_cancel_timeout(ieee802_1x_cp_transmit_when_timeout, sm, NULL
    );
    eloop_register_timeout(sm
                                   ->transmit_when / 1000, 0,
 ...
 * the !electedSelf when KaY is not key server */
    ...
    sm->
            using_receive_sas = false;
    sm->
            server_transmitting = false;
}
...
SM_STATE(CP, READY
    ...
    ieee802_1x_kay_enable_new_info(sm
                                           ->kay);
}
...
SM_STATE(CP, TRANSMIT
    ...
    ieee802_1x_kay_set_latest_sa_attr(sm
                                              ->kay, sm->lki, sm->lan,
                                      sm->ltx, sm->lrx);
    ieee802_1x_kay_enable_tx_sas(sm
                                         ->kay, sm->lki);
    sm->
            all_receiving = false;
    sm->
...
SM_STATE(CP, TRANSMITTING
    ...
    sm->
            retire_when = sm->orx ? sm->retire_delay : 0;
    sm->
            otx = false;
    ieee802_1x_kay_set_old_sa_attr(sm
                                           ->kay, sm->oki, sm->oan,
