...
static const char *sci_txt(const struct ieee802_1x_mka_sci *sci) {
    ...
    os_snprintf(txt, sizeof(txt), MACSTR
                                  "@%u",
...
ieee802_1x_kay_deinit_receive_sc(
    ...
    dl_list_for_each_safe(psa, pre_sa, &psc->sa_list,
                          struct receive_sa,
                          list)ieee802_1x_delete_receive_sa(participant->kay, psa);
...
ieee802_1x_mka_get_live_peer_length(
    ...
    dl_list_for_each(peer, &participant->live_peers,
                     struct ieee802_1x_kay_peer, list)len += sizeof(struct ieee802_1x_mka_peer_id);
...
ieee802_1x_mka_decode_sak_use_body(
     ...
     */
    sa_key = dl_list_first(&participant->sak_list,
...
ieee802_1x_kay_generate_new_sak(struct ieee802_1x_mka_participant *participant) {
    ...
    dl_list_for_each(peer, &participant->live_peers,
                     struct ieee802_1x_kay_peer, list)ctx_len += sizeof(peer->mi);
    ...
    dl_list_for_each(peer, &participant->live_peers,
                     struct ieee802_1x_kay_peer, list)peer->sak_used = false;
...
ieee802_1x_kay_encode_mkpdu(struct ieee802_1x_mka_participant *participant,
    ...
    wpa_printf(MSG_DEBUG, "KaY: Ethernet header: DA="
                          MACSTR
                          " SA="
                          MACSTR
                          " Ethertype=0x%x",
...
ieee802_1x_kay_deinit_transmit_sc(
    ...
    dl_list_for_each_safe(psa, tmp, &psc->sa_list,
                          struct transmit_sa, list)ieee802_1x_delete_transmit_sa(participant->kay, psa);
...
static struct transmit_sa *lookup_txsa_by_an(struct transmit_sc *txsc, u8 an) {
    ...
    dl_list_for_each(txsa, &txsc->sa_list,
                     struct transmit_sa, list) {
        if (txsa->an == an)
            return txsa;
    }
...
int ieee802_1x_kay_create_sas(struct ieee802_1x_kay *kay,
    ...
    latest_sak = NULL;
    dl_list_for_each(sa_key, &principal->sak_list,
                     struct data_key, list) {
...
ieee802_1x_kay_init(struct ieee802_1x_kay_ctx *ctx, enum macsec_policy policy,
    ...
    wpa_printf(MSG_DEBUG, "KaY: Initialize - ifname=%s addr="
                          MACSTR
                          " port=%u priority=%u",
