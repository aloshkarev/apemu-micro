...
eth_p_oui_register(struct hostapd_data *hapd, const char *ifname, u8 oui_suffix,
    ...
    receiver->
            oui_suffix = oui_suffix;
    receiver->
            rx_callback = rx_callback;
    receiver->
            rx_callback_ctx = rx_callback_ctx;

    interfaces = hapd->iface->interfaces;

    dl_list_for_each(iface,
                     &interfaces->eth_p_oui, struct eth_p_oui_iface,
                     list) {
        if (
                os_strcmp(iface
                                  ->ifname, ifname) != 0)
    ...
    if (!found) {
        ...
        os_strlcpy(iface
                           ->ifname, ifname, sizeof(iface->ifname));
        iface->
                l2 = l2_packet_init(ifname, NULL, ETH_P_OUI, eth_p_rx,
                                    iface, 1);
        if (!iface->l2) {
            os_free(iface);
            goto
