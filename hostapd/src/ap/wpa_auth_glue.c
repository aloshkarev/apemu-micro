...
static void hostapd_wpa_auth_psk_failure_report(void *ctx, const u8 *addr) {
    struct hostapd_data *hapd = ctx;
    wpa_msg(hapd->msg_ctx, MSG_INFO, AP_STA_POSSIBLE_PSK_MISMATCH
                                     MACSTR,
            MAC2STR(addr));
}
...
static int hostapd_wpa_auth_send_ether(void *ctx, const u8 *dst, u16 proto,
    ...
    ret = l2_packet_send(hapd->l2, dst, proto, (u8 *)
                                 buf,
                         sizeof(*buf) + data_len);
    os_free(buf);
    return ret;
...
static int hostapd_wpa_auth_update_vlan(void *ctx, const u8 *addr, int vlan_id) {
    ...
    wpa_printf(MSG_INFO,
               "Assigned VLAN ID %d from wpa_psk_file to "
               MACSTR,
...
static void hostapd_request_radius_psk(void *ctx, const u8 *addr, int key_mgmt,
    ...
    wpa_printf(MSG_DEBUG, "RADIUS PSK request for "
                          MACSTR
                          " key_mgmt=0x%x",
