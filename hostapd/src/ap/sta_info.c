...
static void ap_sta_list_del(struct hostapd_data *hapd, struct sta_info *sta) {
    ...
    while (tmp != NULL && tmp->next != sta)
        tmp = tmp->next;
    if (tmp == NULL) {
        wpa_printf(MSG_DEBUG, "Could not remove STA "
                              MACSTR
                              " from "
...
static void ap_sta_hash_del(struct hostapd_data *hapd, struct sta_info *sta) {
    ...
    if (s->hnext != NULL)
        ...
        wpa_printf(MSG_DEBUG, "AP: could not remove STA "
                              MACSTR
...
void ap_free_sta(struct hostapd_data *hapd, struct sta_info *sta) {
    ...
    wpa_printf(MSG_DEBUG, "%s: cancel ap_handle_timer for "
                          MACSTR,
...
void hostapd_free_stas(struct hostapd_data *hapd) {
    ...
    while (sta) {
        ...
        sta = sta->next;
        wpa_printf(MSG_DEBUG, "Removing station "
                              MACSTR,
                   MAC2STR(prev->addr));
...
static void ap_handle_session_timer(void *eloop_ctx, void *timeout_ctx) {
    ...
    wpa_printf(MSG_DEBUG, "%s: Session timer for STA "
                          MACSTR,
...
static int ap_sta_remove(struct hostapd_data *hapd, struct sta_info *sta) {
    ...
    wpa_printf(MSG_DEBUG, "%s: Removing STA "
                          MACSTR
                          " from kernel driver",
               hapd->conf->iface, MAC2STR(sta->addr));
    if (hostapd_drv_sta_remove(hapd, sta->addr) &&
        sta->flags & WLAN_STA_ASSOC) {
        wpa_printf(MSG_DEBUG, "%s: Could not remove station "
                              MACSTR
                              " from kernel driver",
                   hapd->conf->iface, MAC2STR(sta->addr));
        return -1;
...
static void ap_sta_remove_in_other_bss(struct hostapd_data *hapd,
    ...
    for (i = 0; i < iface->num_bss; i++) {
        ...
        wpa_printf(MSG_DEBUG, "%s: disconnect old STA "
                              MACSTR
...
static void ap_sta_disassoc_cb_timeout(void *eloop_ctx, void *timeout_ctx) {
    ...
    wpa_printf(MSG_DEBUG, "%s: Disassociation callback for STA "
                          MACSTR,
               hapd->conf->iface, MAC2STR(sta->addr));
...
void ap_sta_disassociate(struct hostapd_data *hapd, struct sta_info *sta,
                         u16 reason) {
    wpa_printf(MSG_DEBUG, "%s: disassociate STA "
                          MACSTR,
               hapd->conf->iface, MAC2STR(sta->addr));
    sta->last_seq_ctrl = WLAN_INVALID_MGMT_SEQ;
    if (hapd->iface->current_mode &&
...
static void ap_sta_deauth_cb_timeout(void *eloop_ctx, void *timeout_ctx) {
    ...
    wpa_printf(MSG_DEBUG, "%s: Deauthentication callback for STA "
                          MACSTR,
               hapd->conf->iface, MAC2STR(sta->addr));
    ap_sta_remove(hapd, sta);
...
void ap_sta_deauthenticate(struct hostapd_data *hapd, struct sta_info *sta,
    ...
    wpa_printf(MSG_DEBUG, "%s: deauthenticate STA "
                          MACSTR,
               hapd->conf->iface, MAC2STR(sta->addr));
    sta->last_seq_ctrl = WLAN_INVALID_MGMT_SEQ;
...
static void ap_sa_query_timer(void *eloop_ctx, void *timeout_ctx) {
    ...
    wpa_printf(MSG_DEBUG, "%s: SA Query timer for STA "
                          MACSTR
                          " (count=%d)",
...
void ap_sta_disconnect(struct hostapd_data *hapd, struct sta_info *sta,
    ...
    if (sta)
        wpa_printf(MSG_DEBUG, "%s: %s STA "
                              MACSTR
                              " reason=%u",
                   hapd->conf->iface, __func__, MAC2STR(sta->addr),
                   reason);
    else if (addr)
        wpa_printf(MSG_DEBUG, "%s: %s addr "
                              MACSTR
                              " reason=%u",
...
static void ap_sta_delayed_1x_auth_fail_cb(void *eloop_ctx, void *timeout_ctx) {
    ...
    wpa_dbg(hapd->msg_ctx, MSG_DEBUG,
            "IEEE 802.1X: Scheduled disconnection of "
                    MACSTR
...
void ap_sta_delayed_1x_auth_fail_disconnect(struct hostapd_data *hapd,
                                            struct sta_info *sta) {
    wpa_dbg(hapd->msg_ctx, MSG_DEBUG,
            "IEEE 802.1X: Force disconnection of "
                    MACSTR
