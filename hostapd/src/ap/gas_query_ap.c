...
static void gas_query_done(struct gas_query_ap *gas,
                           struct gas_query_pending *query,
                           enum gas_query_ap_result result) {
    wpa_msg(gas->msg_ctx, MSG_INFO, GAS_QUERY_DONE
                                    "addr="
                                    MACSTR
...
void gas_query_ap_deinit(struct gas_query_ap *gas) {
    ...
    dl_list_for_each_safe(query, next, &gas->pending,
                          struct gas_query_pending, list)gas_query_done(gas, query, GAS_QUERY_AP_DELETED_AT_DEINIT);
...
gas_query_get_pending(struct gas_query_ap *gas, const u8 *addr, u8 dialog_token) {
    ...
    dl_list_for_each(q, &gas->pending,
...
void gas_query_ap_tx_status(struct gas_query_ap *gas, const u8 *dst,
    ...
    if (!gas || !gas->current) {
        wpa_printf(MSG_DEBUG, "GAS: Unexpected TX status: dst="
                              MACSTR
    ...
    wpa_printf(MSG_DEBUG, "GAS: TX status: dst="
                          MACSTR
...
static int gas_query_tx(struct gas_query_ap *gas,
    ...
    wpa_printf(MSG_DEBUG, "GAS: Send action frame to "
                          MACSTR
                          " len=%u "
                          "freq=%d prot=%d using src addr "
                          MACSTR,
...
static void gas_query_tx_comeback_timeout(void *eloop_data, void *user_ctx) {
    ...
    wpa_printf(MSG_DEBUG, "GAS: Comeback timeout for request to "
                          MACSTR,
               MAC2STR(query->addr));
...
static void gas_query_tx_comeback_req_delay(struct gas_query_ap *gas,
    ...
    wpa_printf(MSG_DEBUG, "GAS: Send comeback request to "
                          MACSTR
...
int gas_query_ap_rx(struct gas_query_ap *gas, const u8 *sa, u8 categ,
    ...
    if (query == NULL) {
        wpa_printf(MSG_DEBUG, "GAS: No pending query found for "
                              MACSTR
                              " dialog token %u", MAC2STR(sa), dialog_token);
        return -1;
    }

    wpa_printf(MSG_DEBUG, "GAS: Response in %d ms from "
                          MACSTR,
    ...
    if (query->status_code == WLAN_STATUS_QUERY_RESP_OUTSTANDING &&
        ...
        wpa_printf(MSG_DEBUG, "GAS: Query to "
                              MACSTR
                              " dialog token "
    ...
    if (pos + 2 > data + len || pos + 2 + pos[1] > data + len) {
        wpa_printf(MSG_DEBUG, "GAS: No room for Advertisement "
                              "Protocol element in the response from "
                              MACSTR,
                   MAC2STR(sa));
        return 0;
    }
    ...
    if (*pos != WLAN_EID_ADV_PROTO) {
        wpa_printf(MSG_DEBUG, "GAS: Unexpected Advertisement "
                              "Protocol element ID %u in response from "
                              MACSTR,
                   *pos, MAC2STR(sa));
        return 0;
    }
...
static void gas_query_timeout(void *eloop_data, void *user_ctx) {
    ...
    wpa_printf(MSG_DEBUG, "GAS: No response received for query to "
                          MACSTR
                          " dialog token %u",
...
int gas_query_ap_req(struct gas_query_ap *gas, const u8 *dst, int freq,
    ...
    *(
    ...
    wpa_msg(gas
                    ->msg_ctx, MSG_INFO, GAS_QUERY_START "addr=" MACSTR
                                         " dialog_token=%u freq=%d",
            MAC2STR(query
                            ->addr), query->dialog_token, query->freq);
