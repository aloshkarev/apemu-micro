...
void ap_sta_reset_steer_flag_timer(void *eloop_ctx, void *timeout_ctx) {
    ...
    if (sta->agreed_to_steer) {
        wpa_printf(MSG_DEBUG, "%s: Reset steering flag for STA "
                              MACSTR,
...
static void ieee802_11_rx_bss_trans_mgmt_resp(struct hostapd_data *hapd,
    ...
    if (status_code == WNM_BSS_TM_ACCEPT) {
        ...
        eloop_register_timeout(2, 0, ap_sta_reset_steer_flag_timer,
                               hapd, sta);
        wpa_printf(MSG_DEBUG, "WNM: Target BSSID: "
                              MACSTR,
                   MAC2STR(pos));
        wpa_msg(hapd->msg_ctx, MSG_INFO, BSS_TM_RESP
                                         MACSTR
...
static void wnm_beacon_protection_failure(struct hostapd_data *hapd,
    ...
    wpa_msg(hapd->msg_ctx, MSG_INFO, WPA_EVENT_UNPROT_BEACON
                                     "reporter="
                                     MACSTR, MAC2STR(addr));
}
...
static void ieee802_11_rx_wnm_coloc_intf_report(struct hostapd_data *hapd,
    ...
    wpa_msg_ctrl(hapd->msg_ctx, MSG_INFO, COLOC_INTF_REPORT
                                          MACSTR
                                          " %d %s",
...
int ieee802_11_rx_wnm_action_ap(struct hostapd_data *hapd,
    ...
    wpa_printf(MSG_DEBUG, "WNM: Unsupported WNM Action %u from "
                          MACSTR,
...
static void set_disassoc_timer(struct hostapd_data *hapd, struct sta_info *sta,
    ...
    wpa_printf(MSG_DEBUG, "Disassociation timer for "
                          MACSTR
