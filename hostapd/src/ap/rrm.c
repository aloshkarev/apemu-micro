...
static void hostapd_handle_beacon_report(struct hostapd_data *hapd,
    ...
    wpa_printf(MSG_DEBUG, "Beacon report token %u len %zu from "
                          MACSTR,
    ...
    if (wpa_snprintf_hex(report, sizeof(report), pos, len) < 0)
        return;
    wpa_msg(hapd->msg_ctx, MSG_INFO, BEACON_RESP_RX
                                     MACSTR
                                     " %u %02x %s",
...
void hostapd_handle_radio_measurement(struct hostapd_data *hapd,
    ...
    wpa_printf(MSG_DEBUG, "Radio measurement frame, action %u from "
                          MACSTR,
...
int hostapd_send_range_req(struct hostapd_data *hapd, const u8 *addr,
    ...
    wpa_printf(MSG_DEBUG, "Request range: dest addr "
                          MACSTR
...
void hostapd_rrm_beacon_req_tx_status(struct hostapd_data *hapd,
    ...
    if (len < 24 + 3)
        return;
    wpa_msg(hapd->msg_ctx, MSG_INFO, BEACON_REQ_TX_STATUS
                                     MACSTR
