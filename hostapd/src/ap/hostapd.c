...
static void hostapd_reload_bss(struct hostapd_data *hapd) {
    ...
    if ((hapd->conf->wpa || hapd->conf->osen) && hapd->wpa_auth == NULL) {
        ...
        hostapd_setup_encryption(hapd->conf->iface, hapd);
#endif /* CONFIG_WEP */
        hostapd_set_generic_elem(hapd, (u8 *)
                "", 0);
    }
...
static int hostapd_validate_bssid_configuration(struct hostapd_iface *iface) {
    ...
    skip_mask_ext:
    wpa_printf(MSG_DEBUG, "BSS count %lu, BSSID mask "
                          MACSTR
                          " (%d bits)",
    ...
    for (i = 0; i < ETH_ALEN; i++) {
        if ((hapd->own_addr[i] & mask[i]) != hapd->own_addr[i]) {
            wpa_printf(MSG_ERROR, "Invalid BSSID mask "
                                  MACSTR
                                  " for start address "
                                  MACSTR
                                  ".",
...
hostapd_das_disconnect(void *ctx, struct radius_das_attrs *attr) {
    ...
    wpa_printf(MSG_DEBUG, "RADIUS DAS: Found a matching session "
                          MACSTR
...
static int hostapd_setup_bss(struct hostapd_data *hapd, int first,
    ...
    if (!hostapd_drv_none(hapd)) {
        wpa_printf(MSG_DEBUG, "Using interface %s with hwaddr "
                              MACSTR
                              " and ssid \"%s\"",
