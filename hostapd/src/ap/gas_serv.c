...
gas_dialog_create(struct hostapd_data *hapd, const u8 *addr, u8 dialog_token) {
    ...
    if (!sta) {
        ...
        if (!sta) {
            wpa_printf(MSG_DEBUG, "Failed to add STA "
                                  MACSTR
...
gas_serv_dialog_find(struct hostapd_data *hapd, const u8 *addr,
    ...
    sta = ap_get_sta(hapd, addr);
    if (!sta) {
        wpa_printf(MSG_DEBUG, "ANQP: could not find STA "
                              MACSTR,
                   MAC2STR(addr));
        return NULL;
    }
    ...
    wpa_printf(MSG_DEBUG, "ANQP: Could not find dialog for "
                          MACSTR
                          " dialog_token %u", MAC2STR(addr), dialog_token);
    return NULL;
}
...
static struct anqp_element *get_anqp_elem(struct hostapd_data *hapd,
    ...
    dl_list_for_each(elem, &hapd->conf->anqp_elem,
                     struct anqp_element,
                     list) {
