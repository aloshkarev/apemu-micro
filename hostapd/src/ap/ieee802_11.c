...
static int send_auth_reply(struct hostapd_data *hapd, struct sta_info *sta,
    ...
    wpa_printf(MSG_DEBUG, "authentication reply: STA="
                          MACSTR
...
static int ieee802_11_allowed_address(struct hostapd_data *hapd, const u8 *addr,
    ...
    if (res == HOSTAPD_ACL_PENDING) {
        wpa_printf(MSG_DEBUG, "Authentication frame from "
                              MACSTR
...
static void handle_auth(struct hostapd_data *hapd,
    ...
    wpa_printf(MSG_DEBUG, "authentication: STA="
                          MACSTR
                          " auth_alg=%d "
    ...
    if (res == HOSTAPD_ACL_REJECT) {
        wpa_msg(hapd->msg_ctx, MSG_DEBUG,
                "Ignore Authentication frame from "
                MACSTR
...
static int add_associated_sta(struct hostapd_data *hapd,
     ...
     */
    wpa_printf(MSG_DEBUG, "Add associated STA "
                          MACSTR
...
static void handle_disassoc(struct hostapd_data *hapd,
    ...
    wpa_printf(MSG_DEBUG, "disassocation: STA="
                          MACSTR
                          " reason_code=%d",
               MAC2STR(mgmt->sa),
...
static void handle_deauth(struct hostapd_data *hapd,
    ...
    wpa_msg(hapd->msg_ctx, MSG_DEBUG, "deauthentication: STA="
                                      MACSTR
                                      " reason_code=%d",
    ...
    sta = ap_get_sta(hapd, mgmt->sa);
    if (sta == NULL) {
        wpa_msg(hapd->msg_ctx, MSG_DEBUG, "Station "
                                          MACSTR
                                          " trying "
...
static int handle_action(struct hostapd_data *hapd,
    ...
    wpa_printf(MSG_DEBUG, "RX_ACTION category %u action %u sa "
                          MACSTR
                          " da "
                          MACSTR
                          " len %d freq %u",
    ...
    if (mgmt->u.action.category != WLAN_ACTION_PUBLIC &&
        ...
        wpa_printf(MSG_DEBUG, "IEEE 802.11: Ignored Action "
                              "frame (category=%u) from unassociated STA "
                              MACSTR,
    ...
    switch (mgmt->u.action.category) {
        ...
        case WLAN_ACTION_RADIO_MEASUREMENT:
            hostapd_handle_radio_measurement(hapd, (
                    const u8 *) mgmt, len);
            return 1;
    }
...
int ieee802_11_mgmt(struct hostapd_data *hapd, const u8 *buf, size_t len,
    ...
    if (!is_broadcast_ether_addr(mgmt->bssid) &&
        ...
        wpa_printf(MSG_INFO, "MGMT: BSSID="
                             MACSTR
                             " not our address",
                   MAC2STR(mgmt->bssid));
        return 0;
    ...
    if (hapd->iface->state != HAPD_IFACE_ENABLED) {
        wpa_printf(MSG_DEBUG, "MGMT: Ignore management frame while interface is not enabled (SA="
                              MACSTR
                              " DA="
                              MACSTR
                              " subtype=%u)",
...
static void handle_auth_cb(struct hostapd_data *hapd,
    ...
    if (!sta) {
        wpa_printf(MSG_DEBUG, "handle_auth_cb: STA "
                              MACSTR
                              " not found",
                   MAC2STR(mgmt->da));
        return;
...
static void handle_assoc_cb(struct hostapd_data *hapd,
    ...
    sta = ap_get_sta(hapd, mgmt->da);
    if (!sta) {
        wpa_printf(MSG_INFO, "handle_assoc_cb: STA "
                             MACSTR
                             " not found",
                   MAC2STR(mgmt->da));
        return;
...
static void handle_deauth_cb(struct hostapd_data *hapd,
    ...
    sta = ap_get_sta(hapd, mgmt->da);
    if (!sta) {
        wpa_printf(MSG_DEBUG, "handle_deauth_cb: STA "
                              MACSTR
                              " not found", MAC2STR(mgmt->da));
        return;
    }
    if (ok)
        wpa_printf(MSG_DEBUG, "STA "
                              MACSTR
                              " acknowledged deauth",
                   MAC2STR(sta->addr));
    else
...
static void handle_disassoc_cb(struct hostapd_data *hapd,
    ...
    sta = ap_get_sta(hapd, mgmt->da);
    if (!sta) {
        wpa_printf(MSG_DEBUG, "handle_disassoc_cb: STA "
                              MACSTR
                              " not found", MAC2STR(mgmt->da));
        return;
    }
...
static void handle_action_cb(struct hostapd_data *hapd,
    ...
    sta = ap_get_sta(hapd, mgmt->da);
    if (!sta) {
        wpa_printf(MSG_DEBUG, "handle_action_cb: STA "
                              MACSTR
                              " not found", MAC2STR(mgmt->da));
        return;
    }
...
void hostapd_client_poll_ok(struct hostapd_data *hapd, const u8 *addr) {
    ...
    if (sta == NULL)
        return;
    wpa_msg(hapd->msg_ctx, MSG_INFO, AP_STA_POLL_OK
                                     MACSTR,
            MAC2STR(sta->addr));
    if (!(sta->flags & WLAN_STA_PENDING_POLL))
        return;
...
void ieee802_11_rx_from_unknown(struct hostapd_data *hapd, const u8 *src,
    ...
    if (sta && (sta->flags & WLAN_STA_ASSOC_REQ_OK)) {
        wpa_printf(MSG_DEBUG, "Association Response to the STA has "
                              ...
                              "ignore Class 3 frame issue with "
                              MACSTR,
                   MAC2STR(src));
        return;
    }
