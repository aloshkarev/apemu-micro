...
wpa_auth_get_psk(struct wpa_authenticator *wpa_auth,
                 const u8 *addr,
...
static void wpa_sta_disconnect(struct wpa_authenticator *wpa_auth,
    ...
    if (!wpa_auth->cb->disconnect)
        return;
    wpa_printf(MSG_DEBUG, "wpa_sta_disconnect STA "
                          MACSTR
                          " (reason %u)",
               MAC2STR(addr), reason);
...
void wpa_receive(struct wpa_authenticator *wpa_auth,
    ...
    enum {
        PAIRWISE_2, PAIRWISE_4, GROUP_2, REQUEST
    } msg;
    char *msgtxt;
    struct wpa_eapol_ie_parse kde;
    const u8 *key_data;
    ...
    wpa_printf(MSG_DEBUG, "WPA: Received EAPOL-Key from "
                          MACSTR
...
void __wpa_send_eapol(struct wpa_authenticator *wpa_auth,
    ...
    wpa_auth_send_eapol(wpa_auth, sm->addr, (u8 *)
                                hdr, len,
                        sm->pairwise_set);
    os_free(hdr);
}
...
/* Init flag is not cleared here, so avoid busy
    ...
    wpa_auth_set_eapol(sm
                               ->wpa_auth, sm->addr, WPA_EAPOL_portEnabled, 0);
    wpa_remove_ptk(sm);
    wpa_auth_set_eapol(sm
                               ->wpa_auth, sm->addr, WPA_EAPOL_portValid, 0);
    sm->
            TimeoutCtr = 0;
    if (
...
SM_STATE(WPA_PTK, DISCONNECT
    ...
    wpa_sta_disconnect(sm
                               ->wpa_auth, sm->addr, reason);
}
...
SM_STATE(WPA_PTK, AUTHENTICATION
    ...
    sm->
            PTK_valid = false;
    wpa_auth_set_eapol(sm
                               ->wpa_auth, sm->addr, WPA_EAPOL_portControl_Auto,
                       1);
    wpa_auth_set_eapol(sm
                               ->wpa_auth, sm->addr, WPA_EAPOL_portEnabled, 1);
    sm->
...
SM_STATE(WPA_PTK, INITPMK
    ...
    if (sm->pmksa) {
        ...
        os_memcpy(sm
                          ->PMK, sm->pmksa->pmk, sm->pmksa->pmk_len);
        sm->
...
/* IEEE 802.11i does not set keyRun to false, but not doing this
    ...
    wpa_auth_set_eapol(sm
                               ->wpa_auth, sm->addr, WPA_EAPOL_keyRun, false);
}
...
/*
			 ...
			 */
            rsn_pmkid(sm
                              ->PMK, sm->pmk_len, sm->wpa_auth->addr,
...
/* WPA with IEEE 802.1X: use the derived PMK from EAP
    ...
    for (;;) {
        ...
        if (
                wpa_derive_ptk(sm, sm
                                       ->SNonce, pmk, pmk_len, &PTK,
                               owe_ptk_workaround == 2) < 0)
            break;

        if (mic_len &&
            wpa_verify_key_mic(sm
                                       ->wpa_key_mgmt, pmk_len, &PTK,
...
continue;
    ...
    if (!ok &&
        wpa_key_mgmt_wpa_psk_no_sae(sm
                                            ->wpa_key_mgmt) &&
        wpa_auth->conf.radius_psk && wpa_auth->cb->request_radius_psk &&
        !sm->waiting_radius_psk) {
        wpa_printf(MSG_DEBUG,
                   "No PSK available - ask RADIUS server");
        wpa_auth->cb->
                request_radius_psk(wpa_auth
                                           ->cb_ctx, sm->addr,
                                   sm->wpa_key_mgmt,
                                   sm->ANonce,
                                   sm->last_rx_eapol_key,
    ...
    if (!ok) {
        ...
        if (psk_found)
            wpa_auth_psk_failure_report(sm
                                                ->wpa_auth, sm->addr);
        return;
    }
...
/*
    ...
    if (!sm->wpa_ie ||
        wpa_compare_rsn_ie(ft, sm
                                   ->wpa_ie, sm->wpa_ie_len,
                           eapol_key_ie, eapol_key_ie_len)) {
        wpa_auth_logger(wpa_auth, sm
                                ->addr, LOGGER_INFO,
...
/* MLME-DEAUTHENTICATE.request */
        wpa_sta_disconnect(wpa_auth, sm
                                   ->addr,
                           WLAN_REASON_PREV_AUTH_NOT_VALID);
        return;
    }
    if ((!sm->rsnxe && kde.rsnxe) ||
        ...
        (sm->rsnxe && kde.rsnxe &&
         (sm->rsnxe_len != kde.rsnxe_len ||
          os_memcmp(sm
                            ->rsnxe, kde.rsnxe, sm->rsnxe_len) != 0))) {
        wpa_auth_logger(wpa_auth, sm
                                ->addr, LOGGER_INFO,
...
return;
    ...
    if (vlan_id &&
        wpa_key_mgmt_wpa_psk(sm
                                     ->wpa_key_mgmt) &&
        wpa_auth_update_vlan(wpa_auth, sm
                ->addr, vlan_id) < 0) {
        wpa_sta_disconnect(wpa_auth, sm
                                   ->addr,
                           WLAN_REASON_PREV_AUTH_NOT_VALID);
    ...
    sm->
            pending_1_of_4_timeout = 0;
    eloop_cancel_timeout(wpa_send_eapol_timeout, sm
            ->wpa_auth, sm);

    if (
            wpa_key_mgmt_wpa_psk(sm
                                         ->wpa_key_mgmt) && sm->PMK != pmk) {
...
/*
			 ...
			 */
            if (
                    random_get_bytes(stub_gtk, gtk_len
                    ) < 0)
                goto
                        done;
            gtk = stub_gtk;
...
kde_len += 2 + RSN_SELECTOR_LEN + 2;
    ...
    if (
            ocv_oci_add(sm,
                        &pos, conf->oci_freq_override_eapol_m3) < 0)
        goto
                done;
...
/* FIX: MLME-SetProtection.Request(TA, Tx_Rx) */
        ...
        wpa_auth_set_ptk_rekey_timer(sm);
        wpa_auth_store_ptksa(sm
                                     ->wpa_auth, sm->addr, sm->pairwise,
                             dot11RSNAConfigPMKLifetime, &sm->PTK);
    ...
    if (0 /* IBSS == TRUE */) {
        ...
        wpa_auth_set_eapol(sm
                                   ->wpa_auth, sm->addr, WPA_EAPOL_portValid,
                           1);
    }
    wpa_auth_set_eapol(sm
                               ->wpa_auth, sm->addr, WPA_EAPOL_keyAvailable,
                       false);
    wpa_auth_set_eapol(sm
                               ->wpa_auth, sm->addr, WPA_EAPOL_keyDone, true);
    if (sm->wpa == WPA_VERSION_WPA)
        sm->
                PInitAKeys = true;
    else
        sm->
                has_GTK = true;
    wpa_auth_vlogger(sm
                             ->wpa_auth, sm->addr, LOGGER_INFO,
                     "pairwise key handshake completed (%s)",
                     sm->wpa == WPA_VERSION_WPA ? "WPA" : "RSN");
    wpa_msg(sm
                    ->wpa_auth->conf.msg_ctx, MSG_INFO, "EAPOL-4WAY-HS-COMPLETED "
                                                        MACSTR,
            MAC2STR(sm
                            ->addr));
...
SM_STATE(WPA_PTK_GROUP, KEYERROR
    ...
    sm->
            Disconnect = true;
    sm->
            disconnect_reason = WLAN_REASON_GROUP_KEY_UPDATE_TIMEOUT;
    wpa_auth_vlogger(sm
                             ->wpa_auth, sm->addr, LOGGER_INFO,
...
static int wpa_group_disconnect_cb(struct wpa_state_machine *sm, void *ctx) {
    if (sm->group == ctx) {
        wpa_printf(MSG_DEBUG, "WPA: Mark STA "
                              MACSTR
...
int wpa_auth_sta_set_vlan(struct wpa_state_machine *sm, int vlan_id) {
    ...
    wpa_printf(MSG_DEBUG, "WPA: Moving STA "
                          MACSTR
...
void wpa_auth_eapol_key_tx_status(struct wpa_authenticator *wpa_auth,
    ...
    if (!wpa_auth || !sm)
        return;
    wpa_printf(MSG_DEBUG, "WPA: EAPOL-Key TX status for STA "
                          MACSTR
...
void wpa_auth_sta_radius_psk_resp(struct wpa_state_machine *sm, bool success) {
    if (!sm->waiting_radius_psk) {
        wpa_printf(MSG_DEBUG,
                   "Ignore RADIUS PSK response for "
                   MACSTR
    ...
    wpa_printf(MSG_DEBUG, "RADIUS PSK response for "
                          MACSTR
                          " (%s)",
