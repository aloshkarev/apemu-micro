...
int hostapd_dpp_qr_code(struct hostapd_data *hapd, const char *cmd) {
    ...
    if (auth && auth->response_pending &&
        ...
        wpa_msg(hapd->msg_ctx, MSG_INFO, DPP_EVENT_TX
                                         "dst="
                                         MACSTR
                                         " freq=%u type=%d",
...
void hostapd_dpp_tx_status(struct hostapd_data *hapd, const u8 *dst,
    ...
    wpa_printf(MSG_DEBUG, "DPP: TX status: dst="
                          MACSTR
                          " ok=%d",
               MAC2STR(dst), ok);
    wpa_msg(hapd->msg_ctx, MSG_INFO, DPP_EVENT_TX_STATUS
                                     "dst="
                                     MACSTR
...
static void hostapd_dpp_rx_auth_req(struct hostapd_data *hapd, const u8 *src,
    ...
    wpa_printf(MSG_DEBUG, "DPP: Authentication Request from "
                          MACSTR,
               MAC2STR(src));
...
static void hostapd_dpp_handle_config_obj(struct hostapd_data *hapd,
    ...
    wpa_msg(hapd->msg_ctx, MSG_INFO, DPP_EVENT_CONFOBJ_AKM
                                     "%s",
            dpp_akm_str(conf->akm));
    if (conf->ssid_len)
        wpa_msg(hapd->msg_ctx, MSG_INFO, DPP_EVENT_CONFOBJ_SSID
                                         "%s",
    ...
    if (conf->connector) {
         ...
         * message. */
        wpa_msg(hapd->msg_ctx, MSG_INFO, DPP_EVENT_CONNECTOR
                                         "%s",
                conf->connector);
    }
    if (conf->passphrase[0]) {
        ...
        wpa_msg(hapd->msg_ctx, MSG_INFO, DPP_EVENT_CONFOBJ_PASS
                                         "%s",
                hex);
        ...
        wpa_msg(hapd->msg_ctx, MSG_INFO, DPP_EVENT_CONFOBJ_PSK
                                         "%s",
                hex);
    }
    if (conf->c_sign_key) {
...
static void hostapd_dpp_start_gas_client(struct hostapd_data *hapd) {
    ...
    wpa_printf(MSG_DEBUG, "DPP: GAS request to "
                          MACSTR
                          " (freq %u MHz)",
               MAC2STR(auth->peer_mac_addr), auth->curr_freq);
...
static void hostapd_dpp_rx_auth_resp(struct hostapd_data *hapd, const u8 *src,
    ...
    wpa_printf(MSG_DEBUG, "DPP: Authentication Response from "
                          MACSTR,
               MAC2STR(src));
    ...
    if (!is_zero_ether_addr(auth->peer_mac_addr) &&
        ...
        wpa_printf(MSG_DEBUG, "DPP: MAC address mismatch (expected "
                              MACSTR
                              ") - drop", MAC2STR(auth->peer_mac_addr));
        return;
    }
...
static void hostapd_dpp_rx_auth_conf(struct hostapd_data *hapd, const u8 *src,
    ...
    wpa_printf(MSG_DEBUG, "DPP: Authentication Confirmation from "
                          MACSTR,
               MAC2STR(src));
...
static void hostapd_dpp_send_peer_disc_resp(struct hostapd_data *hapd,
    ...
    wpa_printf(MSG_DEBUG, "DPP: Send Peer Discovery Response to "
                          MACSTR
...
static void hostapd_dpp_rx_peer_disc_req(struct hostapd_data *hapd,
    ...
    wpa_printf(MSG_DEBUG, "DPP: Peer Discovery Request from "
                          MACSTR,
               MAC2STR(src));
...
hostapd_dpp_rx_pkex_exchange_req(struct hostapd_data *hapd, const u8 *src,
    ...
    wpa_printf(MSG_DEBUG, "DPP: PKEX Exchange Request from "
                          MACSTR,
               MAC2STR(src));
...
hostapd_dpp_rx_pkex_exchange_resp(struct hostapd_data *hapd, const u8 *src,
    ...
    wpa_printf(MSG_DEBUG, "DPP: PKEX Exchange Response from "
                          MACSTR,
               MAC2STR(src));
    ...
    wpa_printf(MSG_DEBUG, "DPP: Send PKEX Commit-Reveal Request to "
                          MACSTR,
               MAC2STR(src));
...
hostapd_dpp_rx_pkex_commit_reveal_req(struct hostapd_data *hapd, const u8 *src,
    ...
    wpa_printf(MSG_DEBUG, "DPP: PKEX Commit-Reveal Request from "
                          MACSTR,
               MAC2STR(src));
...
hostapd_dpp_rx_pkex_commit_reveal_resp(struct hostapd_data *hapd, const u8 *src,
    ...
    wpa_printf(MSG_DEBUG, "DPP: PKEX Commit-Reveal Response from "
                          MACSTR,
               MAC2STR(src));
...
void hostapd_dpp_rx_action(struct hostapd_data *hapd, const u8 *src,
    ...
    if (crypto_suite != 1) {
        wpa_printf(MSG_DEBUG, "DPP: Unsupported crypto suite %u",
                   crypto_suite);
        wpa_msg(hapd->msg_ctx, MSG_INFO, DPP_EVENT_RX
                                         "src="
                                         MACSTR
    ...
    if (pkex_t >= PKEX_COUNTER_T_LIMIT) {
        wpa_msg(hapd->msg_ctx, MSG_INFO, DPP_EVENT_PKEX_T_LIMIT
                                         "id=0");
...
hostapd_dpp_gas_req_handler(struct hostapd_data *hapd, const u8 *sa,
    ...
    wpa_printf(MSG_DEBUG, "DPP: GAS request from "
                          MACSTR, MAC2STR(sa));
    ...
    wpa_hexdump(MSG_DEBUG,
                ...
                query, query_len);
    wpa_msg(hapd->msg_ctx, MSG_INFO, DPP_EVENT_CONF_REQ_RX
                                     "src="
                                     MACSTR,
            MAC2STR(sa));
