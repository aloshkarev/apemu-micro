...
void ieee802_1x_set_sta_authorized(struct hostapd_data *hapd,
    ...
    if (res && errno != ENOENT) {
        wpa_printf(MSG_DEBUG, "Could not set station "
                              MACSTR
...
static void ieee802_1x_learn_identity(struct hostapd_data *hapd,
    ...
    os_free(sm->identity);
    sm->identity = (u8 *)
            dup_binstr(identity, identity_len);
    if (!sm->identity) {
        sm->identity_len = 0;
        return;
    }
...
void ieee802_1x_receive(struct hostapd_data *hapd, const u8 *sa, const u8 *buf,
    ...
    wpa_printf(MSG_DEBUG, "IEEE 802.1X: %lu bytes from "
                          MACSTR
                          " (encrypted=%d)",
    ...
    if (!sta || (!(sta->flags & (WLAN_STA_ASSOC | WLAN_STA_PREAUTH)) &&
        ...
        if (sta && (sta->flags & WLAN_STA_AUTH)) {
            wpa_printf(MSG_DEBUG, "Saving EAPOL frame from "
                                  MACSTR
    ...
    if (datalen >= sizeof(struct ieee802_1x_eapol_key) &&
        ...
        wpa_receive(hapd->wpa_auth, sta->wpa_sm, (u8 *)
                            hdr,
                    sizeof(*hdr) + datalen);
        return;
    }
...
int ieee802_1x_eapol_tx_status(struct hostapd_data *hapd, struct sta_info *sta,
    ...
    if (len < (int) sizeof(*xhdr))
        return 0;
    wpa_printf(MSG_DEBUG, "IEEE 802.1X: "
                          MACSTR
